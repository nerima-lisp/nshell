...
- Added the public-readiness bar (now
  `docs/src/project/public-readiness.md`) to make the world-level release
  bar, evidence requirements, and remaining public gaps explicit.
- Documentation site (`docs/mkdocs.yml`, `docs/src/`) built by `packages.docs`
  with `mkdocs build --strict` and published by `docs.yml`. Because the build is
  also wired in as `checks.docs`, a broken link or a page missing from the nav
  now fails the pull request rather than the post-merge deploy.
- `run-tests.lisp` at the repository root as the single Lisp-level test entry
  point, invoked by `checks.default`, `apps.test` (`nix run .#test`), and the
  dev-shell `test` alias, so the hermetic check and a local run execute the same
  code path instead of three hand-rolled `--eval` chains that drift apart.
- `docs.yml` and `flake-update.yml` workflows, and a `.github/actions/nix-setup`
  composite action that pins the nix-installer and Cachix SHAs in one place for
  every workflow. `flake-update.yml` opens a weekly PR bumping the flake inputs,
  which is what keeps the nixpkgs pin from silently aging.
- `checks.formatting`, a treefmt (nixfmt) gate scoped to Nix sources only, plus
  `nix fmt` via `formatter`. YAML and Markdown are deliberately out of scope: a
  YAML formatter rewrites the GitHub Actions `on:` key, and reformatting
  Markdown would churn the entire docs tree.
...
- The contribution guide and the PR checklist now distinguish hermetic checks from
  non-sandboxed OS-interactive integration coverage.
- The contribution guide now includes a release checklist for CI gates, binary smoke
...
- Conformed the repository to the nerima-lisp package standard. The test
  directory moved from `tests/` to `t/`; `nshell.asd` gained the mandatory
  `:maintainer`, `:bug-tracker`, and `:source-control` metadata on every system
  and now uses the canonical `:author`; and the CI, docs, release, and
  flake-update workflows follow the org templates.
- `flake.nix` now derives every version from a `.asd` file rather than
  hardcoding it. nshell's own version comes from `nshell.asd`, making that form
  the single source of truth that `release.yml` checks a tag against, and each
  dependency's `buildASDFSystem` version is read from its own `.asd` — the
  hand-written strings had drifted (cl-log-kit's real 1.0.0 was labelled
  "1.6.0", cl-process-kit's 1.0.0 was "0.2.0").
- Pinned all eight remaining nerima-lisp flake inputs to release tags. A bare
  `github:nerima-lisp/<name>` follows that repository's default branch, so an
  unrelated upstream merge could turn this repository's CI red without any
  change here.
- Narrowed the flake's declared `systems` to `x86_64-linux` and
  `aarch64-darwin`, dropping `aarch64-linux` and `x86_64-darwin`. Nothing
  verified either of the two removed platforms. The `binary` CI job and the
  release matrix drop their macos-13 (x86_64-darwin) leg to match, so release
  tarballs now cover the two platforms that are actually built and started in
  CI rather than advertising a third that was never checked.
- Every workflow now declares `permissions:`, gives each job a
  `timeout-minutes`, and pins every action to a 40-character commit SHA.
  `DeterminateSystems/nix-installer-action@main` in particular was a mutable
  branch reference: whatever that branch pointed at ran with the workflow's
  privileges.
- `release.yml` now verifies that the pushed tag matches `nshell.asd`'s
  `:version` and that `nix flake check` passes before building anything, and
  takes its release body from the matching `CHANGELOG.md` section. The
  three-platform binary distribution (tarball plus SHA-256 checksum, with the
  man page) is unchanged apart from the dropped x86_64-darwin target.
- Deduplicated `flake.nix`: the dependency library definitions and the
  `CL_SOURCE_REGISTRY` string were maintained separately in `packages` and
  `checks`, which is how cl-log-kit and cl-process-kit came to be missing from
  the checks' registry. Both are now defined once.
...
- `run-external-times-out-and-returns` now binds `*standard-output*` to a
  string stream, so it exercises the timeout-bearing path it means to.
  `%foreground-external-command-timeout` applies `*external-command-timeout*`
  only when stdout is not an interactive terminal, and SBCL reports its
  standard streams as interactive even when they are attached to a pipe, so the
  test had been asserting a timeout on a call that -- correctly, under the new
  gating -- was never given one. It only surfaced once `nix flake check` could
  evaluate on aarch64-darwin again.
...
- `CONTRIBUTING.md` and `SECURITY.md` from the repository root. GitHub serves
  the org-wide versions from `nerima-lisp/.github` to any repository that omits
  them, so a per-repository copy is a second thing to keep in sync. The
  nshell-specific parts — the layer rules, test-selection table, quality bar,
  release checklist, and security scope — moved to
  `docs/src/project/contributing.md` and `docs/src/project/releasing.md`.
- The nine working notes under `docs/` (coverage analysis, CPS, dead-code,
  readability, timeout, macro-consolidation, value-struct and package audits,
  paredit usage) are no longer part of the documentation site. They are
  engineering records rather than reader-facing documentation, and now live in
  `docs/notes/`, outside `docs_dir`.
