...
実装済みの範囲:
- 最大ステップ数の既定値は 10。`NSHELL_AI_MAX_STEPS` に正の整数を設定すると上書きできる。
- `q` は agent セッションを停止し、実行中のコマンドが exit 130 で中断された場合はステップを `interrupted` として記録する。
- agent 由来の実行履歴は history v3 の `agent` origin で記録する。非対話経路では sidecar を起動せず exit 2 を返す。
