...
| 沈黙が既定 | 明示トリガーがない限りモデルは呼ばれない。失敗時も「1 キーで聞ける」だけ | one-shot 3.8s、常駐でも 1.3–1.5s/turn という計測値では、打鍵ごとの ambient 提案は成立しない |
...
- モデルは shell の外でコマンドを実行できない (`--tools ""` で sidecar のツールを無効化し、実行は常に nshell 側)。
