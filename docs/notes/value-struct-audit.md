...
| `%type-options` | `application/builtin-type-helpers.lisp` | converted — immutable option value threaded through the parser |
...
| `arithmetic-substitution` | `domain/expansion/arithmetic.lisp` | converted — immutable value; parser helper renamed to avoid accessor collision |
...
| `brace-expansion-frame` | `domain/expansion/brace.lisp` | converted — immutable expansion frame |
| `case-node` | `domain/parsing/ast.lisp` | include-hierarchy — `:include ast-node`; the macro cannot generate an inheriting struct |
| `command-list-redirect-split-result` | `domain/parsing/ast-redirect-split.lisp` | converted — immutable split result |
| `command-node` | `domain/parsing/ast.lisp` | include-hierarchy — `:include ast-node`; the macro cannot generate an inheriting struct |
| `command-redirect-split-result` | `domain/parsing/ast-redirect-split.lisp` | converted — immutable split result |
| `completion-context` | `domain/completion/context.lisp` | converted — immutable completion input context |
| `completion-query` | `domain/completion/query-data.lisp` | converted — immutable completion request value |
| `config` | `domain/configuration/config.lisp` | converted — immutable value object with the public `theme` reader generated by `define-value-struct` |
| `theme` | `domain/configuration/theme.lisp` | converted — immutable color mapping with copy-on-write updates |
| `env-var` | `domain/environment/env.lisp` | converted — immutable value with defensive copying for structured values |
...
| `job-wait-event` | `application/manage-job-data.lisp` | converted — immutable wait event value |
...
| `token` | `domain/parsing/tokenizer-data.lisp` | converted — immutable lexical token value |
| `tokenizer-state` | `domain/parsing/tokenizer-data.lisp` | mutable — writable slot(s): input, len, cursor-pos, pos, tokens, incomplete |
| `vi-input-transition` | `presentation/input-state-vi-data.lisp` | converted — immutable transition value |
...
| `%complete-parse-state` | `application/builtin-complete.lisp` | immutable parser state; option application returns fresh state |

The remaining raw `defstruct` entries are lifecycle state, mutable parser state, or
AST include hierarchies; they are intentionally outside the value-struct migration.
