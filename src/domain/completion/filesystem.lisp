...
(defun %join-directory-command (directory command &key (empty-directory directory))
  (let ((dir
        (if (string= directory "") empty-directory
          directory)))
    (cond
      ((string= dir "") command)
...
(defun command-path-candidates (command path executable-p &key (empty-directory ""))
  ...
  (if (%command-prefix-has-directory-p command)
      (when (funcall executable-p command) (list command))
...
(defun %first-command-path-candidate (command path executable-p
                                      &key (empty-directory ""))
  "Return the first executable COMMAND projected through PATH."
  (if (%command-prefix-has-directory-p command)
      (and (funcall executable-p command) command)
      (let ((path (or path ""))
            (start 0))
        (loop for pos = (position #\: path :start start)
              for directory = (subseq path start pos)
...
(defun %executable-candidate-p (entry)
  (ignore-errors
    (or
      (null *path-command-executable-p-fn*)
...
(defun %trim-trailing-path-separators (text)
  ...
  (let ((end (length text)))
    (loop while (and (> end 1) (%path-separator-p (char text (1- end))))
          do (decf end))
    (subseq text 0 end)))
  ...
  (let ((name (pathname-name path))
    ...
    (when (and name (not (keywordp name)))
      (let ((base (princ-to-string name)))
        (when (< 0 (length base))
          (if (and type (not (eq type :unspecific)) (not (keywordp type))) (concatenate 'string base "." (princ-to-string type))
            base))))))
...
(defun %entry-path-name (entry)
  ...
  (cond
    ((pathnamep entry)
      (or
        (%pathname-file-component-name entry)
    ...
    ((stringp entry)
      (let* ((trimmed (%trim-trailing-path-separators entry))
        ...
        (if separator (subseq trimmed (1+ separator))
          trimmed)))
    (t nil)))
...
(defstruct (%file-completion-prefix-projection
    ...
    (:conc-name %file-completion-prefix-projection-)) (directory-prefix "" :type string :read-only t)
...
(defun %project-file-completion-prefix (prefix)
  ...
  (let ((separator (position-if #'%path-separator-p prefix :from-end t)))
    (if separator (%make-file-completion-prefix-projection
...
(defun %ensure-directory-candidate-suffix (text)
  ...
  (if (or (string= text "") (%path-separator-p (char text (1- (length text))))) text
...
(defstruct (%path-command-query
    ...
    (:conc-name %path-command-query-)) (path nil :type (or null string) :read-only t)
...
(defstruct (%file-completion-query
    ...
    (:conc-name %file-completion-query-)) (directory-prefix "" :type string :read-only t)
...
(defun %path-command-query-active-p (query)
  (and
    *path-command-directory-files-fn*
    ...
    (not (%command-prefix-has-directory-p (%path-command-query-prefix query)))))
...
(defvar *path-command-cache-clock-fn* (lambda ()
    (/
...
(defstruct (%path-command-directory-cache-entry
    ...
    (:conc-name %path-command-directory-cache-entry-)) stamp
...
(progn
  ...
  (defvar *path-command-directory-key-locks* (make-hash-table :test (quote equal)))
  (defun %require-path-command-cache-lock-factory ()
    (or
      *path-command-cache-lock-factory*
  ...
  (defun %require-path-command-directory-cache-lock ()
    (or
      *path-command-directory-cache-lock*
  ...
  (defun %configure-path-command-cache-locks (factory)
    ...
    (setf *path-command-cache-lock-factory* factory
          ...
          *path-command-directory-key-locks* (make-hash-table :test (quote equal))
          *path-command-directory-cache* (make-hash-table :test (quote equal)))
  ...
  (%with-path-command-directory-cache-lock
    (let ((record (gethash key *path-command-directory-key-locks*)))
      (if record (progn
          (incf (cdr record))
          record)
        (setf (gethash key *path-command-directory-key-locks*) (cons (%make-path-command-cache-lock) 1))))))
...
(defmacro %with-path-command-directory-key-lock ((key) &body body)
  (let ((key-value (gensym "KEY"))
    ...
    `(let* ((,key-value ,key)
      ...
      (unwind-protect (cl-boundary-kit:call-with-lock
          (car ,record)
          (lambda ()
            ,@body))
...
(defun %path-command-directory-cache-get (key stamp now)
  (%with-path-command-directory-cache-lock
    (let ((entry (gethash key *path-command-directory-cache*)))
      (if (%path-command-directory-cache-valid-p entry stamp now) (values (copy-list (%path-command-directory-cache-entry-entries entry)) t)
        (values nil nil)))))
...
(defun %path-command-directory-cache-put (key generation stamp now entries)
  (%with-path-command-directory-cache-lock
    (when (= generation *path-command-directory-cache-generation*)
      (when (>=
          (hash-table-count *path-command-directory-cache*)
          *path-command-cache-limit*)
        (clrhash *path-command-directory-cache*))
      (setf (gethash key *path-command-directory-cache*) (%make-path-command-directory-cache-entry stamp now (copy-list entries)))))
  entries)
...
(defun %list-path-command-directory (directory)
  (let* ((pathname (%path-command-directory-pathname directory))
    ...
    (labels ((lookup ()
               (let ((now (coerce (funcall *path-command-cache-clock-fn*) (quote double-float)))
            ...
            (%path-command-directory-cache-get key stamp now)))
             (scan ()
               (let* ((generation (%path-command-directory-cache-generation))
                 ...
                 (now (coerce (funcall *path-command-cache-clock-fn*) (quote double-float))))
            (when (equal stamp-before stamp-after)
              (%path-command-directory-cache-put key generation stamp-after now entries))
            entries)))
      (multiple-value-bind (cached foundp) (lookup)
        (if foundp cached
          (%with-path-command-directory-key-lock
            (key)
            (multiple-value-bind (locked-cached locked-foundp) (lookup)
              (if locked-foundp locked-cached
                (scan)))))))))

(defun %path-command-entry-candidate (entry prefix executable-p)
  (let ((name (%entry-command-name entry)))
    (when (and name (%starts-with-p prefix name) (funcall executable-p entry))
    ...
    (if (not (%path-command-query-active-p query))
        ...
        (let* ((query-prefix (%path-command-query-prefix query))
          ...
          (%path-command-candidates-from-entries
            entries-by-directory
            query-prefix
            (function %executable-candidate-p))))))

(defun %file-completion-query-from-prefix (prefix include-files include-directories)
  (multiple-value-bind (directory-prefix name-prefix) (%split-file-completion-prefix prefix)
    (%make-file-completion-query
      directory-prefix
      name-prefix
...
(defun %file-completion-entry-candidate (entry kind query)
  (let ((name (%entry-path-name entry)))
    (when (and
      ...
      (let* ((raw-text
            (concatenate 'string (%file-completion-query-directory-prefix query) name))
             (text
            (if (eq kind :directory) (%ensure-directory-candidate-suffix raw-text)
              raw-text)))
        (make-candidate
          text
          :kind
          kind
          :score
          (if (eq kind :directory) 70
            60)
          :description
          (if (eq kind :directory) "directory"
            "file"))))))
...
(defun %file-candidates-from-directory (prefix &key (include-files t) (include-directories t))
  ...
  (let ((query
        (%file-completion-query-from-prefix prefix include-files include-directories))
  ...
  (ecase mode
    (:directories
      (%file-candidates-from-directory
        ...
        t))
    (:files-and-directories
      (%file-candidates-from-directory prefix :include-files t :include-directories t))))
