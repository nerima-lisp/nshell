...
(defun %consume-here-doc-body (input start delimiter &optional strip-tabs-p)
  (let ((delimiter-text (if (consp delimiter)
                            (car delimiter)
                            delimiter))
        (strip-tabs-p (or strip-tabs-p
                          (and (consp delimiter)
                               (cdr delimiter)))))
    (with-output-to-string (body)
      (loop with pos = start
            ...
            do (let* ((line (%read-here-doc-line input pos))
                      (raw-line-text (%here-doc-line-text line))
                      (line-text
                        (if strip-tabs-p
                            (string-left-trim '(#\Tab) raw-line-text)
                            raw-line-text)))
                 (when (string= line-text delimiter-text)
                   (return-from %consume-here-doc-body
                     (%make-here-doc-body
                      (get-output-stream-string body)
                      (%here-doc-line-next-position line)
                      nil)))
                 (write-string line-text body)
