...
(defmethod nshell.domain.completion:predicate-true-p ((predicate (eql 'test-builtin-true)) args bindings)
  (declare (ignore predicate args bindings))
  t)

(defmethod nshell.domain.completion:predicate-true-p ((predicate (eql 'test-builtin-string=)) args bindings)
  (declare (ignore predicate bindings))
  (and (= 2 (length args)) (string= (first args) (second args))))

(describe
  "completion-rules-tests"
  (it
    "fact-only-resolution"
