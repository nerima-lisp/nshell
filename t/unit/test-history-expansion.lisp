...
(describe "history-expansion-domain-tests"
  ...
  (it "reports-unresolvable-designators-and-literal-boundaries"
    "Recognized but unresolved designators report an error, while delimiter boundaries remain literal."
    (with-history (history)
      (dolist (case '( ("!!" "history expansion failed: !!")
                       ("!$" "history expansion failed: !$")
                       ("!-0" "history expansion failed: !-0")
                       ("!-1" "history expansion failed: !-1")
                       ("!?missing?" "history expansion failed: !?missing?")
                       ("!??" "history expansion failed: !??")
                       ("!?missing" "history expansion failed: !?missing")
                       ("!unknown" "history expansion failed: unknown")))
        (multiple-value-bind (expanded error)
            (nshell.domain.history:history-expand-line history (first case))
          (expect expanded :to-be-null)
          (expect (second case) :to-equal error))))
    (with-history (history "echo first")
      (multiple-value-bind (expanded error)
          (nshell.domain.history:history-expand-line history "echo !")
        (expect "echo !" :to-equal expanded)
        (expect error :to-be-null))
      (multiple-value-bind (expanded error)
          (nshell.domain.history:history-expand-line history "echo ! next")
        (expect "echo ! next" :to-equal expanded)
        (expect error :to-be-null)))))
