...
(describe "completion-rules-tests"
  (it "fact-only-resolution"
    (let ((kb (make-empty-rule-kb)))
      ...
      (expect 1 :to-equal (length (nshell.domain.completion:prove-all kb '(completes "ls" "--help"))))))

  (it "rule-knowledge-base-constructor-is-internal-boundary"
    ...
    (expect (nshell.domain.completion::rule-knowledge-base-p
         (nshell.domain.completion::make-empty-rule-knowledge-base)) :to-be-truthy))

  (it "fact-and-rule-constructors-validate-domain-values"
    (assert-package-function-boundaries
        :package "NSHELL.DOMAIN.COMPLETION"
    ...
    (expect (lambda () (nshell.domain.completion:make-fact :predicate "completes"
                                          :args '("ls" "--help"))) :to-throw 'type-error)
    (expect (lambda () (nshell.domain.completion:make-fact :predicate 'completes
                                          :args "ls")) :to-throw 'type-error)
    (expect (lambda () (nshell.domain.completion:make-rule :head "bad"
                                          :body '())) :to-throw 'type-error)
    (expect (lambda () (nshell.domain.completion:make-rule :head '(suggests-dir ?input)
                                          :body "bad")) :to-throw 'type-error)
    (let* ((args (list "ls" "--help"))
      ...
      (expect '(suggests-dir ?input) :to-equal (nshell.domain.completion::rule-head rule))
      (expect '((command-is ?input "cd")) :to-equal (nshell.domain.completion::rule-body rule))))

  (it "rule-with-one-body-goal-resolves"
    (let ((kb (make-empty-rule-kb)))
      (nshell.domain.completion:assert-fact!
       kb
  ...
  (it "rule-with-conjunction-resolves"
    (let ((kb (make-empty-rule-kb)))
      (nshell.domain.completion:assert-fact!
       kb
      ...
      (nshell.domain.completion:assert-rule!
       ...
       (nshell.domain.completion:make-rule :head '(documented-command ?cmd)
                                            :body '((command-is ?cmd "git")
                                                    (has-flag ?cmd "--help"))))
      (expect 1 :to-equal (length (nshell.domain.completion:prove-all kb '(documented-command "git"))))))

  (it "rule-disjunction-via-multiple-rules"
    (let ((kb (make-empty-rule-kb)))
      (nshell.domain.completion:assert-fact!
       kb
      ...
      (let ((solutions (nshell.domain.completion:prove-all kb '(completes "git" ?sub))))
        ...
        (expect (member "add" (mapcar (lambda (solution) (solution-binding '?sub solution)) solutions)
                    :test #'string=) :to-be-truthy)
        (expect (member "commit" (mapcar (lambda (solution) (solution-binding '?sub solution)) solutions)
                    :test #'string=) :to-be-truthy))))

  (it "variable-binding-extraction"
    (let ((kb (make-empty-rule-kb)))
      (nshell.domain.completion:assert-fact!
       kb
       (nshell.domain.completion:make-fact :predicate 'completes :args '("ls" "--help")))
  ...
  (it "rule-completion-projects-solution-values-through-candidates"
    (let ((kb (make-empty-rule-kb)))
      (nshell.domain.completion:assert-fact!
       kb
      ...
      (let ((candidates (nshell.domain.completion:rule-complete kb "gi")))
        ...
        (expect "git" :to-equal (nshell.domain.completion:candidate-text (first candidates)))
        (expect "version control" :to-equal (nshell.domain.completion:candidate-description (first candidates))))))

  (it "rule-solution-projections-are-internal-boundaries"
    (assert-package-function-boundaries
        :package "NSHELL.DOMAIN.COMPLETION"
  ...
  (it "rule-data-spec-builders-feed-proof-search"
    (let ((kb (nshell.domain.completion::make-empty-rule-knowledge-base)))
      ...
      (let ((solutions (nshell.domain.completion:prove-all
        ...
        (expect 1 :to-equal (length solutions))
        (expect "status" :to-equal (solution-binding '?completion (first solutions))))))

  (it "rule-data-spec-builders-are-internal-boundaries"
    (assert-package-function-boundaries
        :package "NSHELL.DOMAIN.COMPLETION"
